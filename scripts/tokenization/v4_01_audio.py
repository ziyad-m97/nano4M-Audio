"""v4 audio tokenization + scene_desc, in one pass.

Per stem:
  - Load audio.wav (24 kHz mono, ~81920 samples)
  - EnCodec encode @ 1.5 kbps → codes [K=2, T=256], interleave → 512 ids in [0,1023]
  - Tile K=10× → [10, 512] int32 (per friend's note: keep int32 for cross-modality
    consistency, even though int16 fits)
  - Write {out}/{split}/tok_audio@512/{stem}.npy
  - Write {out}/{split}/scene_desc/{stem}.json = ["<class_name>"] * 10

Resumable: skips files with correct shape/dtype.
"""
...
from v4_layout import (
    iter_stems, audio_path, out_path, atomic_save_npy, atomic_write_text,
)
...
def encode(model: EncodecModel, wav: torch.Tensor) -> np.ndarray:
    """Return [10, 512] int32 interleaved + tiled."""
    with torch.no_grad():
        frames = model.encode(wav)
    codes = torch.cat([f[0] for f in frames], dim=-1)   # [1, K_cb, T]
    assert codes.shape[1] == EXPECT_CODEBOOKS, f"K={codes.shape[1]}"
    assert codes.shape[2] == EXPECT_FRAMES, f"T={codes.shape[2]}"
    arr = codes[0].cpu().numpy()                         # [K_cb=2, T=256]
    interleaved = arr.T.reshape(-1)                      # [T*K_cb=512]
    assert interleaved.shape[0] == TOK_PER_SLOT
    assert interleaved.min() >= 0 and interleaved.max() < 1024
...
def main() -> int:
    ...
    model = model.to("cpu").eval()   # encodec is small + int-op-heavy → CPU fine
    ...
    for i, (sp, cls, stem_dir) in enumerate(items, 1):
        ...
        if not already_done_desc(d_out):
            atomic_write_text(d_out, json.dumps([cls] * K))
            stats["done_desc"] += 1
